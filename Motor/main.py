...
def disable_1(NodeNr):
    NodeNr.disable()
    print('Disable axis ' + str(NodeNr) + ' successful')

def goPos(NodeNr):
    NodeNr.enable()
    NodeNr.posEnable()

C = Controller("/dev/ttyUSB0")
# C.ClearDigOut(1)   #Power on Hall sensors
# print("Device Type: ", C.getCastedRegister(0x1000))
# print("Serial Number: ", C.getCastedRegister(0x1018, subindex=4))
# print("Status: ", C.getCastedRegister(0x6041))
...
X1.setTarget(100000)
# goPos(X1)
print("Pos Set Complete")
# disable_1 must wait action end for time
# disable_1(X1)
print("")

r=np.arange(0,5*2*np.pi,0.1)
s=np.sin(r)
actual_pos=[]
for i in s:
    X1.setTarget(i*100000)
    actual_pos.append(X1.getPosition())
for j in range(10):
    for i in range(10):
        if(j%2==0):
            d=1
